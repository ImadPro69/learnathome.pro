...
if ( ! class_exists( 'Themosaurus_Merlin' ) ) :
	...
	abstract class Themosaurus_Merlin {
		...
		public function __construct() {
			...
			if ( is_admin() && class_exists( 'TGM_Plugin_Activation' ) && isset( $_GET['demo'] ) ) {
				...
				if ( function_exists( 'set_time_limit' ) ) {
					set_time_limit( 600 ); // Raise time limit during import for slow hosts
				}
		...
		/**
		 * Change the page template for a set of given pages
		 *
		 * @param array $pages Array of pages where the key is the page slug, and the value is the page template
		 */
		protected function change_pages_template( $pages ) {
			if ( ! is_array( $pages ) ) {
				return;
			}

			foreach ( $pages as $page_slug => $page_template ) {
		 ...
		 *
		 * @param array $menus Array containing menu locations as keys and menu names as values.
		 * E.g.: array( 'primary' => 'Primary', 'secondary' => 'Secondary' )
		 */
		...
		/**
		 * Change WooCommerce attributes settings
		 *
		 * @param array $attributes
		 */
		public function change_wc_attributes_settings( $attributes ) {
			...
			foreach ( $attributes as $attribute_name => $attribute_settings ) {
				$attribute = wc_attribute_taxonomy_id_by_name( $attribute_name );

				if ( empty( $attribute ) || ! is_array( $attribute_settings ) ) {
					continue;
				}

				wc_update_attribute( $attribute, $attribute_settings );
			}
		}
		...
		/**
		 * Change the selected icon kit for a specified menu
		 *
		 * @param array $menus Name of the menu to change
		 * @param string $icon_kit Name of the icon kit to use for the menu
		 */
		public function change_menus_icon_kit( $menus, $icon_kit ) {
			if ( empty( WPMI_DB_KEY ) || empty( $icon_kit ) ) {
				return;
			}
			...
			foreach ( $menus as $menu_name ) {
				...
				update_term_meta( $menu->term_id, WPMI_DB_KEY, $icon_kit );
			}
		}
		...
		public function init() {
			new Merlin(
				...
				$strings = apply_filters( 'themosaurus_merlin_strings', array(
					...
					'demo'                     => esc_html__( 'You can import some demo content to help you get started with the theme. To get started, please choose your demo right here:', 'themosaurus-merlin' ),
