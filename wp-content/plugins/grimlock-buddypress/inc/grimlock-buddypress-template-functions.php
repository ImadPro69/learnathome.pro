...
if ( ! function_exists( 'grimlock_buddypress_member_get_send_message_button_args' ) ) :
	...
	function grimlock_buddypress_member_get_send_message_button_args() {
		if ( function_exists( 'bp_loggedin_user_url' ) ) {
			$messages_url = bp_loggedin_user_url( bp_members_get_path_chunks( array( bp_get_messages_slug(), 'compose' ) ) );
		}
		else {
		...
		$user_id   = bp_get_member_user_id();
		$link_href = apply_filters( 'grimlock_buddypress_member_send_message_button_url', wp_nonce_url( $messages_url . '?r=' . bp_core_get_username( $user_id ) ), $user_id );
