...
class Grimlock_BuddyPress_Customizer extends Grimlock_Base_Customizer {
	...
	public function __construct() {
		...
		add_filter( 'grimlock_buddypress_navbar_nav_menu_item_avatar_displayed',             array( $this, 'is_navbar_nav_menu_item_avatar_displayed'             ), 10, 1 );
	...
	public function add_customizer_fields() {
		$this->defaults = apply_filters( 'grimlock_buddypress_customizer_defaults', array(
			'navbar_nav_menu_item_logged_out_avatar_displayed'  => true,
			'navbar_nav_menu_item_logged_in_avatar_displayed'   => true,
		...
		// Navigation Tab
		$this->add_navbar_nav_menu_item_logged_out_avatar_displayed_field(          array( 'priority' => 10 ) );
		$this->add_navbar_nav_menu_item_logged_in_avatar_displayed_field(           array( 'priority' => 20 ) );
		if ( bp_is_active( 'friends' ) ) {
			$this->add_navbar_nav_menu_item_friends_displayed_field(                array( 'priority' => 30 ) );
		}
		if ( bp_is_active( 'groups' ) ) {
			$this->add_navbar_nav_menu_item_groups_displayed_field(                 array( 'priority' => 40 ) );
		}
		if ( bp_is_active( 'notifications' ) ) {
			$this->add_navbar_nav_menu_item_notifications_displayed_field(          array( 'priority' => 50 ) );
			if ( class_exists( 'BuddyDev_BP_Notifications_Widget_Helper' ) ) {
				$this->add_navbar_nav_menu_item_notifications_list_displayed_field( array( 'priority' => 60 ) );
			}
		}
		if ( bp_is_active( 'messages' ) ) {
			$this->add_navbar_nav_menu_item_messages_displayed_field(               array( 'priority' => 70 ) );
		}
		if ( bp_is_active( 'settings' ) ) {
			$this->add_navbar_nav_menu_item_settings_displayed_field(               array( 'priority' => 80 ) );
		}
		$this->add_navbar_nav_menu_tooltips_enabled_field(                          array( 'priority' => 90 ) );
	}
	...
	public function add_customizer_controls_js_data( $js_data ) {
		$js_data['tabs'][ $this->section ] = array(
			array(
				...
				'controls' => array(
					'navbar_nav_menu_item_logged_out_avatar_displayed',
					'navbar_nav_menu_item_logged_in_avatar_displayed',
	...
	/**
	 * Add a Kirki checkbox field in the Customizer to set whether the avatar in the navbar is shown while logged out
	 *
	 * @param array $args The array of arguments for the Kirki field.
	 */
	protected function add_navbar_nav_menu_item_logged_out_avatar_displayed_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			$args = wp_parse_args( $args, array(
				'type'     => 'checkbox',
				'section'  => $this->section,
				'label'    => esc_html__( 'Display Navbar Avatar Menu for Logged Out Users', 'grimlock-buddypress' ),
				'settings' => 'navbar_nav_menu_item_logged_out_avatar_displayed',
				'default'  => $this->get_default( 'navbar_nav_menu_item_logged_out_avatar_displayed' ),
				'priority' => 20,
			) );

			Kirki::add_field( 'grimlock', apply_filters( 'grimlock_buddypress_navbar_nav_menu_item_logged_out_avatar_displayed_field_args', $args ) );
		}
	}

	/**
	 * Add a Kirki checkbox field in the Customizer to set whether the avatar in the navbar is shown while logged in
	 *
	 * @param array $args The array of arguments for the Kirki field.
	 */
	protected function add_navbar_nav_menu_item_logged_in_avatar_displayed_field( $args = array() ) {
		if ( class_exists( 'Kirki') ) {
			$args = wp_parse_args( $args, array(
				'type'     => 'checkbox',
				'section'  => $this->section,
				'label'    => esc_html__( 'Display Navbar Avatar Menu for Logged In Users', 'grimlock-buddypress' ),
				'settings' => 'navbar_nav_menu_item_logged_in_avatar_displayed',
				'default'  => $this->get_default( 'navbar_nav_menu_item_logged_in_avatar_displayed' ),
				'priority' => 20,
			) );

			Kirki::add_field( 'grimlock', apply_filters( 'grimlock_buddypress_navbar_nav_menu_item_logged_in_avatar_displayed_field_args', $args ) );
		}
	}
	...
	/**
	 * Check whether navbar avatar need to be displayed.
	 *
	 * @param  bool $default The value for the avatar display.
	 *
	 * @return bool
	 */
	public function is_navbar_nav_menu_item_avatar_displayed( $default ) {
		$logged_out_displayed = $this->get_theme_mod( 'navbar_nav_menu_item_logged_out_avatar_displayed' );
		$logged_in_displayed  = $this->get_theme_mod( 'navbar_nav_menu_item_logged_in_avatar_displayed' );

		if ( ( ! is_user_logged_in() && ! $logged_out_displayed ) || ( is_user_logged_in() && ! $logged_in_displayed ) ) {
			return false;
		}
