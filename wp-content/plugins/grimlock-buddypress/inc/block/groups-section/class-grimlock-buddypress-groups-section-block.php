...
class Grimlock_BuddyPress_Groups_Section_Block extends Grimlock_Section_Block {
	...
	public function __construct( $type = 'groups-section', $domain = 'grimlock-buddypress' ) {
		...
		// Query Panel
		add_filter( "{$this->id_base}_query_panel_fields", array( $this, 'add_max_groups_field'     ), 100, 2 );
		add_filter( "{$this->id_base}_query_panel_fields", array( $this, 'add_group_default_field'  ), 110, 2 );

		// Layout Panel
		add_filter( "{$this->id_base}_layout_panel_fields", array( $this, 'add_groups_layout_field'  ), 90,  2 );
	}
	...
	public function get_defaults() {
		...
		if ( function_exists( 'bp_rewrites_get_url' ) ) {
			$link = bp_rewrites_get_url( array( 'component_id' => 'groups' ) );
		}
		else if ( function_exists( 'bp_get_groups_directory_permalink' ) ) {
