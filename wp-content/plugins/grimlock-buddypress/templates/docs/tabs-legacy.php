		...
		<ul class="d-flex align-items-center flex-wrap">
			...
			<?php if ( is_user_logged_in() ) : ?>
				<?php if ( function_exists( 'bp_is_group' ) && bp_is_group() ) : ?>
					<li id="bp_docs_mydocs_personal" <?php if ( bp_is_current_action( bp_docs_get_docs_slug() ) ) : ?> class="current"<?php endif ?>><a href="<?php bp_group_permalink( groups_get_current_group() ) ?><?php bp_docs_slug() ?>"><?php printf( __( "%s's Docs", 'buddypress-docs' ), bp_get_current_group_name() ) ?></a></li>
				<?php else : ?>
