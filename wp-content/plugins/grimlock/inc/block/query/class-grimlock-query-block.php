...
/**
 * Class Grimlock_Query_Block
 *
 * @author  themosaurus
 * @since   1.1.0
 * @package grimlock/inc
 */
class Grimlock_Query_Block extends Grimlock_Base_Block {
	 ...
	 * @since 1.3.5
	 */
	public function __construct( $type = 'query', $domain = 'grimlock' ) {
		parent::__construct( $type, $domain );

		// Query tab
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_site_field'           ), 100 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_post_type_field'      ), 110 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_handpick_posts_field' ), 120 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_posts_field'          ), 130 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_separator'            ), 140 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_taxonomies_field'     ), 150 );
		...
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_posts_per_page_field' ), 200 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_separator'            ), 210 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_use_meta_query_field' ), 220 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_meta_key_field'       ), 230 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_meta_compare_field'   ), 240 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_meta_value_field'     ), 250 );
		add_filter( "{$this->id_base}_query_panel_fields",      array( $this, 'add_meta_value_num_field' ), 260 );

		// Layout tab
		add_filter( "{$this->id_base}_layout_panel_fields",     array( $this, 'add_posts_layout_field'          ), 100 );
		add_filter( "{$this->id_base}_layout_panel_fields",     array( $this, 'add_auto_slide_enabled_field'    ), 110 );
		add_filter( "{$this->id_base}_layout_panel_fields",     array( $this, 'add_slider_loop_enabled_field'   ), 115 );
		add_filter( "{$this->id_base}_layout_panel_fields",     array( $this, 'add_slider_arrows_enabled_field' ), 120 );
		add_filter( "{$this->id_base}_layout_panel_fields",     array( $this, 'add_slider_pagination_field'     ), 130 );
		add_filter( "{$this->id_base}_layout_panel_fields",     array( $this, 'add_slider_transition_field'     ), 140 );
	...
	public function add_taxonomies_field( $fields ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			$terms = $this->get_taxonomy_hierarchy( $taxonomy->name );

			if ( ! empty( $terms ) ) {
				$taxonomies_choices[ $taxonomy->name ] = array(
					'label'      => $taxonomy->label,
					'subchoices' => $this->build_terms_choices_array( $terms ),
				);
			}
		}
	 ...
	 * @return array
	 */
	protected function build_terms_choices_array( $terms, $terms_choices = array(), $prefix = '' ) {
		foreach ( $terms as $term ) {
			$terms_choices[ $term->taxonomy . '|' . $term->slug ] = ! empty( $prefix ) ? $prefix . ' ' . $term->name : $term->name;

			if ( ! empty( $term->children ) ) {
				$terms_choices = $this->build_terms_choices_array( $term->children, $terms_choices, $prefix . '-' );
			}
		}

		return $terms_choices;
	}
	 ...
	 * @return array
	 */
	protected function get_taxonomy_hierarchy( $taxonomy, $parent = 0 ) {
		// get all direct descendants of the $parent
		$terms = get_terms( array( 'taxonomy' => $taxonomy, 'parent' => $parent ) );
			...
			'conditional_logic' => array(
				array(
					...
					array(
						'field'    => 'posts_layout',
						'operator' => '==',
						'value'    => '12-cols-classic-slider',
					),
					array(
						'field'    => 'posts_layout',
						'operator' => '==',
						'value'    => '6-6-cols-classic-slider',
					),
					array(
						'field'    => 'posts_layout',
						'operator' => '==',
						'value'    => '4-4-4-cols-classic-slider',
					),
					array(
						'field'    => 'posts_layout',
						'operator' => '==',
						'value'    => '3-3-3-3-cols-classic-slider',
					),
				)
			),
		) ) );
	...
	/**
	 * Add a toggle field to set whether to enable slider loop for slider layouts
	 *
	 * @param array $fields Array of block fields
	 *
	 * @return array Modified array of block fields
	 */
	public function add_slider_loop_enabled_field( $fields ) {
		$fields[] = $this->toggle_field( apply_filters( "{$this->id_base}_slider_loop_enabled_field_args", array(
			'name'  => 'slider_loop_enabled',
			'label' => esc_html__( 'Loop slider', 'grimlock' ),
			'conditional_logic' => array(
				array(
					array(
						'field'    => 'posts_layout',
	...
	/**
	 * Add a toggle field to set whether to enable arrows in slider layouts
	 *
	 * @param array $fields Array of block fields
	 *
	 * @return array Modified array of block fields
	 */
	public function add_slider_arrows_enabled_field( $fields ) {
		$fields[] = $this->toggle_field( apply_filters( "{$this->id_base}_slider_arrows_enabled_field_args", array(
			'name'  => 'slider_arrows_enabled',
			'label' => esc_html__( 'Display slider arrows', 'grimlock' ),
			'conditional_logic' => array(
				array(
					array(
						'field'    => 'posts_layout',
	...
	/**
	 * Add a toggle field to set the type of slider pagination in slider layouts
	 *
	 * @param array $fields Array of block fields
	 *
	 * @return array Modified array of block fields
	 */
	public function add_slider_pagination_field( $fields ) {
		$fields[] = $this->select_field( apply_filters( "{$this->id_base}_slider_pagination_field_args", array(
			'name'  => 'slider_pagination',
			'label' => esc_html__( 'Slider pagination', 'grimlock' ),
			'choices'           => array(
				''            => esc_html__( 'None', 'grimlock' ),
				'bullets'     => esc_html__( 'Dots', 'grimlock' ),
				'progressbar' => esc_html__( 'Progressbar', 'grimlock' ),
			),
			'conditional_logic' => array(
				array(
					array(
	...
	/**
	 * Add a toggle field to set the type of slider transition in slider layouts
	 *
	 * @param array $fields Array of block fields
	 *
	 * @return array Modified array of block fields
	 */
	public function add_slider_transition_field( $fields ) {
		$fields[] = $this->select_field( apply_filters( "{$this->id_base}_slider_transition_field_args", array(
			'name'              => 'slider_transition',
			'label'             => esc_html__( 'Slider transition animation', 'grimlock' ),
			'choices'           => array(
				'slide' => esc_html__( 'Slide', 'grimlock' ),
				'fade'  => esc_html__( 'Fade', 'grimlock' ),
			),
			'conditional_logic' => array(
				array(
					array(
	...
	public function get_defaults() {
		return array(
			...
			'slider_loop_enabled'   => false,
			'slider_arrows_enabled' => true,
			'slider_pagination'     => '',
	...
	public function render_block( $attributes, $content ) {
		...
		ob_start();
		do_action( 'grimlock_query', apply_filters( "{$this->id_base}_component_args", $this->get_component_args( $attributes ), $attributes ) );
		return ob_get_clean();
	}
	...
	protected function get_component_args( $attributes ) {
		return array(
			...
			'slider_loop_enabled'   => $attributes['slider_loop_enabled'],
			'slider_arrows_enabled' => $attributes['slider_arrows_enabled'],
			'slider_pagination'     => $attributes['slider_pagination'],
			'slider_transition'     => $attributes['slider_transition'],
	...
	public function sanitize_attributes( $new_attributes ) {
		...
		$attributes['slider_loop_enabled']   = isset( $new_attributes['slider_loop_enabled'] ) && filter_var( $new_attributes['slider_loop_enabled'], FILTER_VALIDATE_BOOLEAN );
		$attributes['slider_arrows_enabled'] = isset( $new_attributes['slider_arrows_enabled'] ) && filter_var( $new_attributes['slider_arrows_enabled'], FILTER_VALIDATE_BOOLEAN );
		$attributes['slider_pagination']     = isset( $new_attributes['slider_pagination'] ) ? sanitize_text_field( $new_attributes['slider_pagination'] ) : '';
		$attributes['slider_transition']     = isset( $new_attributes['slider_transition'] ) ? sanitize_text_field( $new_attributes['slider_transition'] ) : '';
...
return new Grimlock_Query_Block();
